...
Reconstruction::Reconstruction(MeshBlock *pmb, ParameterInput *pin) :
    characteristic_projection{false}, weno_flag{false},
    ...
    correct_err{pin->GetOrAddBoolean("time", "correct_err", false)}, pmy_block_{pmb}
{
  ...
  // TODO(tomo-ono): Even when we use WENO3, is ngh>=3 required?
  if (xorder == 3) {
    int req_nghost = 3;
  ...
  if ((xorder == 3) || (xorder == 4)) {
    ...
    Real **beta = new Real*[4];
    for (int i=0; i<4; ++i) {
      beta[i] = new Real[4];
    }
    Real w_sol[4], b_prhs[4], b_mrhs[4];
    int permute[4];
        ...
        for (int i=(pmb->is)-NGHOST+2; i<=(pmb->ie)+NGHOST-1; ++i) {
          ...
          if (!uniform[X1DIR]) {  // nonuniform spacing in curvilinear coordinate
            for (int row=0; row<4; row++) {
              b_mrhs[row] = std::pow(pco->x1f(i), row);
              for (int col=0; col<4; col++) {
                ...
                int s = row - 2;  // rescale index from -2 to +1
            ...
            if (DoolittleLUPDecompose(beta, 4, permute)) {
              // LUP decomposition succeeded
              DoolittleLUPSolve(beta, permute, b_mrhs, 4, w_sol);
            } else {
              std::stringstream msg;
              msg << "### FATAL ERROR in Reconstruction constructor" << std::endl
                  << "Failure in LUP decomposition step for computing PPMx1 \n"
                  << "curvilinear reconstruction weights at cell index i="
                  << i << " for nx1=" << pmb->block_size.nx1 << std::endl;
              std::cout << msg.str();
              ATHENA_ERROR(msg);
            }
            if (i < pmb->is) {
            ...
            if (std::strcmp(COORDINATE_SYSTEM, "cylindrical") == 0) {
              ...
              // w_im1
              c1i(i) = -(2.0*io - 3.0)*(5.0*SQR(io)*io + 8.0*SQR(io)
                                       - 3.0*io - 4.0)/delta;
              // w_i
              ...
              c3i(i) = (2.0*io + 1.0)*(35.0*SQR(io)*io - 24.0*SQR(io)
                                       - 93.0*io + 60.0)/delta;
              // w_ip2
              c4i(i) = -(2.0*io + 3.0)*(5.0*SQR(io)*io
                                       - 8.0*SQR(io) - 3.0*io + 4.0)/delta;
            } else { // if (std::strcmp(COORDINATE_SYSTEM, "spherical_polar") == 0)
      ...
      if (uniform[X1DIR]) {
        if (curvilinear[X1DIR]) {
          for (int i=(pmb->is)-NGHOST+2; i<=(pmb->ie)+NGHOST-1; ++i) {
            Real io = std::abs(i - pmy_block_->is);  // il=is-1 ---> io = 2
            if (std::strcmp(COORDINATE_SYSTEM, "cylindrical") == 0) {
              Real delta = 12.0*(SQR(io)-io-1.0)*(2.0*io-1.0);
              // wp_1
              c1i(i) = (2.0*io+1.0)*(4.0*SQR(io)-9.0*io+4.0)/delta;
              // wm_1
              c2i(i) =-(2.0*io+1.0)*(2.0*SQR(io)-4.0*io+1.0)/delta;
            } else { //spherical polar
              Real delta = 18.0*(10.0*SQR(SQR(SQR(io)))-30.0*SQR(SQR(io))*io
                                 +15.0*SQR(SQR(io))+20.0*SQR(io)*io-9.0*SQR(io)
                                 -6.0*io+4.0);
              // wp_1
              c1i(i) = 2.0*(3.0*SQR(io)+3.0*io+1.0)
                       *(10.0*SQR(SQR(io))-45.0*SQR(io)*io
                         +72.0*SQR(io)-48.0*io+12.0)/delta;
              // wm_1
              c2i(i) =-(3.0*SQR(io)+3.0*io+1.0)
                       *(10.0*SQR(SQR(io))-40.0*SQR(io)*io
                         +51.0*SQR(io)-22.0*io+4.0)/delta;
            }
          }
        } else { // cartesian-like
#pragma omp simd
        ...
        for (int i=(pmb->is)-NGHOST+2; i<=(pmb->ie)+NGHOST-1; ++i) {
          const Real rc = pco->x1v(i);
          for (int row=0; row<3; row++) {
            b_prhs[row] = std::pow(pco->x1f(i+1)-rc, row);
            b_mrhs[row] = std::pow(pco->x1f(i)-rc, row);
            int s = row - 1;  // rescale index from -2 to +1
            const Real rp = pco->x1f(i+s+1);
            const Real rm = pco->x1f(i+s  );
            Real vol;
            if (std::strcmp(COORDINATE_SYSTEM, "cartesian") == 0) {
              vol = rp-rm;
              ...
              vol = ONE_3RD*(rp*rp*rp-rm*rm*rm);
            }
            for (int col=0; col<3; col++) {
              if (std::strcmp(COORDINATE_SYSTEM, "cartesian") == 0) {
                beta[col][row] = (pow(rp-rc, col+1.0)-pow(rm-rc, col+1.0))
                                 /((col+1.0)*vol);
              } else if (std::strcmp(COORDINATE_SYSTEM, "cylindrical") == 0) {
                beta[col][row] = (pow(rp-rc, col+1.0)*((col+1.0)*rp+rc)
                                  -pow(rm-rc, col+1.0)*((col+1.0)*rm+rc))
                                 /((col+2.0)*(col+1.0)*vol);
              } else { // spherical_polar
                beta[col][row] = (pow(rp-rc, col+1.0)*((col*col+3.0*col+2.0)*rp*rp
                                                       +2.0*rc*(col+1.0)*rp+2.0*rc*rc)
                                  -pow(rm-rc, col+1.0)*((col*col+3.0*col+2.0)*rm*rm
                                                       +2.0*rc*(col+1.0)*rp+2.0*rc*rc))
                                 /((col+3.0)*(col+2.0)*(col+1.0)*vol);
              }
            }
          }
          if (!DoolittleLUPDecompose(beta, 3, permute)) {
            std::stringstream msg;
            msg << "### FATAL ERROR in Reconstruction constructor" << std::endl
                << "Failure in LUP decomposition step for computing WENOx1\n"
                << "reconstruction weights at cell index i="
                << i << " for nx1=" << pmb->block_size.nx1 << std::endl;
            std::cout << msg.str();
            ATHENA_ERROR(msg);
          }
          DoolittleLUPSolve(beta, permute, b_prhs, 3, w_sol);
          c1i(i) = w_sol[2];
          DoolittleLUPSolve(beta, permute, b_mrhs, 3, w_sol);
          c2i(i) = w_sol[2];
        }
      }
    } // end x1
    ...
    if (pmb->block_size.nx2 > 1) {
      ...
      if (!weno_flag) {
        if (curvilinear[X2DIR]) {
          ...
          factorial = [&factorial] (int k)
                      -> int {return (k <=1) ? 1 : factorial(k-1)*k;};
          for (int j=(pmb->js)-NGHOST+2; j<=(pmb->je)+NGHOST-1; ++j) {
            // need to numerically invert system of equations given by Mignone
            // equations 21 and 27 with specific nonuniform or uniform meridional
            // mesh spacings in order to precompute reconstruction weights
            for (int row=0; row<4; row++) {
              b_mrhs[row] = std::pow(pco->x2f(j), row);
              int s = row - 2;  // rescale index from -2 to +1
            ...
            if (DoolittleLUPDecompose(beta, 4, permute)) {
              ...
              std::stringstream msg;
              msg << "### FATAL ERROR in Reconstruction constructor" << std::endl
                  << "Failure in LUP decomposition step for computing PPMx2\n"
                  << "curvilinear reconstruction weights at cell index j="
                  << j << " for nx2=" << pmb->block_size.nx2 << std::endl;
              std::cout << msg.str();
              ATHENA_ERROR(msg);
            }
...
#pragma omp simd
        ...
        if (curvilinear[X2DIR]) {
          ...
          w[2] = 5.0/9.0;
          for (int j=(pmb->js)-NGHOST+2; j<=(pmb->je)+NGHOST-1; ++j) {
            const Real tc = pco->x2v(j);
            for (int row=0; row<3; row++) {
              b_mrhs[row] = std::pow(pco->x2f(j)-tc, row);
              b_prhs[row] = std::pow(pco->x2f(j+1)-tc, row);
              int s = row - 1;  // rescale index from -2 to +1
              const Real tp = pco->x2f(j+s+1);
              const Real tm = pco->x2f(j+s  );
              const Real vol = std::cos(tm) - std::cos(tp);
              for (int col=0; col<3; col++) {
                Real int_temp = 0.0;
                for (int nn=0; nn<3; nn++) {
                  Real tt = 0.5*(tp-tm)*z[nn]+0.5*(tp+tm);
                  int_temp += w[nn]*std::pow(tt-tc, col)*std::sin(tt);
                }
                int_temp *= 0.5*(tp-tm);
                beta[col][row] = int_temp/vol;
              }
            }
            if (!DoolittleLUPDecompose(beta, 3, permute)) {
              std::stringstream msg;
              msg << "### FATAL ERROR in Reconstruction constructor" << std::endl
                  << "Failure in LUP decomposition step for computing WENOx2\n"
...
#pragma omp simd
            ...
            for (int j=(pmb->js)-NGHOST+2; j<=(pmb->je)+NGHOST-1; ++j) {
              const Real rc = pco->x2v(j);
              for (int row=0; row<3; row++) {
                b_prhs[row] = std::pow(pco->x2f(j+1)-rc, row);
                b_mrhs[row] = std::pow(pco->x2f(j)-rc, row);
                int s = row - 1;  // rescale index from -2 to +1
                const Real rp = pco->x2f(j+s+1);
                const Real rm = pco->x2f(j+s  );
                const Real vol = rp-rm;
                for (int col=0; col<3; col++) {
                  beta[col][row] = (pow(rp-rc, col+1.0)-pow(rm-rc, col+1.0))
                                   /((col+1.0)*vol);
                }
              ...
              if (!DoolittleLUPDecompose(beta, 3, permute)) {
                ...
                msg << "### FATAL ERROR in Reconstruction constructor" << std::endl
                    << "Failure in LUP decomposition step for computing WENOx2\n"
                    << "reconstruction weights at cell index j="
                    << j << " for nx2=" << pmb->block_size.nx2 << std::endl;
                std::cout << msg.str();
                ATHENA_ERROR(msg);
              }
...
#pragma omp simd
          ...
          for (int k=(pmb->ks)-NGHOST+2; k<=(pmb->ke)+NGHOST-1; ++k) {
            const Real rc = pco->x3v(k);
            for (int row=0; row<3; row++) {
              b_prhs[row] = std::pow(pco->x3f(k+1)-rc, row);
              b_mrhs[row] = std::pow(pco->x3f(k)-rc, row);
              int s = row - 1;  // rescale index from -2 to +1
              const Real rp = pco->x3f(k+s+1);
              const Real rm = pco->x3f(k+s  );
              const Real vol = rp-rm;
              for (int col=0; col<3; col++) {
                beta[col][row] = (pow(rp-rc, col+1.0)-pow(rm-rc, col+1.0))
                                 /((col+1.0)*vol);
            ...
            if (!DoolittleLUPDecompose(beta, 3, permute)) {
              std::stringstream msg;
              msg << "### FATAL ERROR in Reconstruction constructor" << std::endl
                  << "Failure in LUP decomposition step for computing WENOx3\n"
                  << "reconstruction weights at cell index k="
                  << k << " for nx3=" << pmb->block_size.nx3 << std::endl;
              std::cout << msg.str();
              ATHENA_ERROR(msg);
            }
            DoolittleLUPSolve(beta, permute, b_prhs, 3, w_sol);
